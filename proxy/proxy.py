...
import requests
import threading
import remote_code_execution_detection_linux
...
def manager():
    ...
    processes_detection_thread = threading.Thread(target=remote_code_execution_detection_linux.run)
    requests_detection_thread = threading.Thread(target=bruteForce_dos_sessionPrediction_detection.check_request)

    # devices detection
    device_detection_thread = threading.Thread(target=devices_detection.find_new_device)
    usb_detection_thread = threading.Thread(target=devices_detection.find_new_usb)
